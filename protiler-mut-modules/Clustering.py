...
def clustering(
    ...
    corr_vals = [
        corr_dic[df_gene["sgID"][ix]] if df_gene["sgID"][ix] in corr_dic else 0
        for ix in g.dendrogram_col.reordered_ind
    ]
    ...
    plt.subplot(1, 3, 1)
    plt.scatter(embedding[:, 0], embedding[:, 1], c=top_colors_list, s=50)
    ...
    plt.subplot(1, 3, 3)
    plt.scatter(tsne_result[:, 0], tsne_result[:, 1], c=top_colors_list, s=50)
